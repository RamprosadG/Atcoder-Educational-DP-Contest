...
void update(int pos, vi& not_taken, ll score, int mask, int group) {
	if (pos == sz(not_taken)) {
		smax(dp[mask], score + pre[group]);
		return;
	}
	update(pos + 1, not_taken, score, mask, group);
	update(pos + 1, not_taken, score, mask ^ (1 << not_taken[pos]), group ^ (1 << not_taken[pos]));
}
...
int solve() {
	...
	dp.resize(1 << n, -Inf), pre.resize(1 << n);
	dp[0] = 0;
	...
	for (int mask = 0; mask < (1 << n); mask++) {
		...
		update(0, not_taken, dp[mask], mask, 0);
	}
	printf("%lld\n", dp[(1 << n) - 1]);
	return 0;
}
